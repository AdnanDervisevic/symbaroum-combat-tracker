...
Phase:        ALL EIGHT COMPLETE. The port is done and green.
              ...
              2026-08-24 - GITHUB ACTIONS IS NOT AVAILABLE on this account. The
...
Next action:  Nothing is blocking. What is left is in "Open questions" below, and
              the two that matter are both HUMAN steps, not code:
                ...
                2. Decide whether the live Vercel URL should point at this
                   branch. `./scripts/deploy_vercel.sh` publishes the built site
                   to the `vercel-deploy` branch, which Vercel serves at that
                   branch's own URL. Promoting it to production is one setting in
                   the Vercel dashboard. **`master` and the live deployment have
                   not been touched.**
