...
let%expect_test "test_npc_naming_survives_removal" =
  let world = add_goblins World.initial ~count:3 ~ids:[ "c1"; "c2"; "c3" ] in
...
let%expect_test "test_sort_preserves_round" =
  let world =
    add_goblins World.initial ~count:3 ~ids:[ "c1"; "c2"; "c3" ]
...
let%expect_test "test_delete_character_repairs_cursor" =
  let ids =
    List.map Default_roster.ids ~f:(fun id -> id, cid (Ids.Character_id.to_string id))
  in
  let world =
    fst (World.apply World.initial (Add_player_characters { characters = ids }))
  in
  ...
  let world =
    fst (World.apply world (Delete_character { id = chid "pc_default_thalia" }))
  in
  printf "after:  ";
  print_s
    [%sexp
...
let%expect_test "test_typing_a_note_costs_one_undo" =
  let world = add_goblins World.initial ~count:1 ~ids:[ "c1" ] in
